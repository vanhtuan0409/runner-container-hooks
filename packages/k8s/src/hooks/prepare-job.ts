...
export async function prepareJob(
  ...
  let prepareScript = prepareJobScript(args.container.userMountVolumes || []);
...
function generateResponseFile(
  ...
  if (args.services?.length) {
    const serviceContainerNames = args.services?.map((s) =>
      generateContainerName(s.image)
    ) || [];
        ...
        if (c.ports?.length) {
          for (const port of c.ports) {
            if (port.containerPort && port.hostPort) {
              ctxPorts[port.containerPort.toString()] = port.hostPort
                .toString();
            }
          }
        }
